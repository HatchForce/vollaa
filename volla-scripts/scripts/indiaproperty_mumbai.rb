...
def get_properties(i)
  puts "####"+" "+"#{i}" +" "+"####"
  agent = Mechanize.new
  page = agent.get("http://mumbai.indiaproperty.com/property/mumbai.html?page=#{i}")
  @data_container = []
  ...
  x.each do |x|
    ...
    more_link = "http://mumbai.indiaproperty.com" + div_child.children()[2].children()[2].children()[0].children()[2].children()[0].attributes['href'] rescue ""
    created_at = Time.now
    #puts x
  ...
  File.open("indianproperty_mumbai#{@timestamp}.txt", 'a+') do |f|
