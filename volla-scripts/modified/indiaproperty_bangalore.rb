...
def get_properties(i)
  puts "####"+" "+"#{i}" +" "+"####"
  agent = Mechanize.new
  page = agent.get("http://bangalore.indiaproperty.com/property/bangalore.html?page=#{i}")
  @data_container = []
  ...
  x.each do |x|
    ...
    more_link = "http://bangalore.indiaproperty.com" + div_child.children()[2].children()[2].children()[0].children()[2].children()[0].attributes['href'] rescue ""
    created_at = Time.now
    puts x
  ...
  File.open("indianproperty_bangalore_#{@timestamp}.txt", 'a+') do |f|
