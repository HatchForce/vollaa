...
def get_properties(i)
  puts "####"+" "+"#{i}" +" "+"####"
  agent = Mechanize.new
  page = agent.get("http://chennai.indiaproperty.com/property/chennai.html?page=#{i}")
  @data_container = []
  ...
  x.each do |x|
    ...
    more_link = "http://chennai.indiaproperty.com" + div_child.children()[2].children()[2].children()[0].children()[2].children()[0].attributes['href'] rescue ""
    created_at = Time.now
    puts x
  ...
  File.open("indiaproperty_chennai#{@timestamp}.txt", 'a+') do |f|
